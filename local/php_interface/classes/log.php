...
Class Log {
    ...
    public function set($dirname, $message, $error = false){
        ...
        $string .= ( ( $error === true )?"Ошибка: ":( ($error === false)?"":$error ) );
        $string .= $message;

        file_put_contents(dirname(__FILE__)."/../logs/".$dirname."/".$name.".txt", $string."\n", FILE_APPEND);

        if( $error === true )
            file_put_contents(dirname(__FILE__)."/../logs/errors.txt", $string."\n", FILE_APPEND);
    }
